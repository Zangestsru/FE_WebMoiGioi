...
const ProfilePage: React.FC = () => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      showStatus(
        "Thành công",
        "Thông tin hồ sơ của bạn đã được cập nhật đồng bộ.",
        "success",
      );
    } else {
      showStatus(
        "Lỗi cập nhật",
        result.message || "Có lỗi xảy ra khi lưu hồ sơ.",
        "error",
      );
    }
  };
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.success) {
      showStatus(
        "Thành công",
        "Ảnh đại diện của bạn đã được cập nhật đồng bộ.",
        "success",
      );
    } else {
      showStatus(
        "Lỗi tải ảnh",
        result.message || "Không thể tải ảnh lên hệ thống.",
        "error",
      );
    }
  };
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showStatus(
          "Đã gửi mã",
          "Mã xác nhận bảo mật đã được gửi đến Gmail của bạn.",
          "success",
        );
      }
    } catch (error) {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hasPassword) {
        const { response, errors: localErrors } =
        ...
        const { response, errors: localErrors } =
          await userService.setPassword(pwdForm);
        if (localErrors) {
          setErrors(localErrors);
          return;
        }
        if (response?.success) {
          showStatus(
            "Thành công",
            "Thiết lập mật khẩu riêng cho tài khoản thành công.",
            "success",
          );
          setHasPassword(true);
          setPwdForm({
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <div className="lg:col-span-7 space-y-10">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h2 className="text-3xl font-bold text-gray-900 font-primary tracking-tight">
                      Cài đặt hồ sơ
                    </h2>
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-sm font-bold text-gray-700 mb-2 font-primary">
                        Tiểu sử cá nhân
                      </label>
                      <textarea
                    ...
                    <FormInput
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          zaloContactPhone: e.target.value,
                        })
                      }
              ...
              <div className="lg:col-span-5 space-y-8 bg-gray-50/50 p-8 rounded-[32px] border border-gray-100/50">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-2 font-primary">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-blue-100 mt-1 text-sm leading-relaxed">
                          Đăng nhập bằng Google hiện chưa có mật khẩu riêng. Bạn
                          nên đặt mật khẩu mới ngay để tăng tối đa tính an toàn.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-end gap-3">
                      <div className="flex-1">
                        <FormInput
                          ...
                          value={pwdForm.otp}
                          onChange={(e) =>
                            setPwdForm({ ...pwdForm, otp: e.target.value })
                          }
